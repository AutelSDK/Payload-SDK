...
static void record_adc_data(uint8_t ch, uint16_t data)
{
    switch(ch) {
    case ADC1_IN8_CH:
		samp_data.NTC_R = ((float)INPUT_VOL*DOWN_REGISTOR*ADC_STEP)/(data * REVERF_VOL)-DOWN_REGISTOR;//K
        break;
    case ADC1_IN3_CH:
...
void adc_sample_task(void)
{
    ...
    else 
    {
		...
		if (ADC_GetStatus(ADC_UNIT, ADC_EOC_FLAG) == SET) 
		{
				...
				record_adc_data(adc_channels[ch_index], u16AdcValue);
