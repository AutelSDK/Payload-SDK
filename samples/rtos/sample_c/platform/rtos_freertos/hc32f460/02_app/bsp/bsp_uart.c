...
#define UART1_READ_BUF_SIZE      12
#define UART1_WRITE_BUF_SIZE     12
#define UART2_READ_BUF_SIZE      12
#define UART2_WRITE_BUF_SIZE     12 
#define UART3_READ_BUF_SIZE      12
#define UART3_WRITE_BUF_SIZE     12
...
//UART1 mutex
static T_AutelMutexHandle s_uart1Mutex;
//USART1 handle
static stc_usart_uart_init_t s_uart1Handle;
#endif
...
static T_AutelMutexHandle s_uart2Mutex;
...
static T_AutelMutexHandle s_uart3Mutex;
...
void UART_Init(E_UartNum uartNum, uint32_t baudRate)
{
    switch (uartNum) {
        ...
        case UART_NUM_1: {
            ...
            USART_UART_StructInit(&s_uart1Handle);
            s_uart1Handle.u32Baudrate = baudRate;
            USART_UART_Init(&s_uart1Handle);
...
{
    if (Autel_CONSOLE_UART_NUM == UART_NUM_1) {
        HAL_UART_Transmit(&s_uart1Handle, (uint8_t *) &ch, 1, 0xFFFF);
    } else if (Autel_CONSOLE_UART_NUM == UART_NUM_2) {
        HAL_UART_Transmit(&s_uart2Handle, (uint8_t *) &ch, 1, 0xFFFF);
    } else if (Autel_CONSOLE_UART_NUM == UART_NUM_3) {
