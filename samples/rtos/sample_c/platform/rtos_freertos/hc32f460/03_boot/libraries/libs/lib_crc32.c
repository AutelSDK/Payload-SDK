 ...
 *
 * This file is a part of AUTEL:
 *
 *   Copyright (C) 2016 AUTEL. All rights reserved.
 *
...
uint32_t lib_crc32part(FAR const uint8_t *src, size_t len, uint32_t crc32val)
{
	size_t i;
...
uint32_t lib_crc32(FAR const uint8_t *src, size_t len)
{
	return lib_crc32part(src, len, 0);
}
